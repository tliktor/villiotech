...
export default function Lakossagnak() {
  ...
  return (
    <>
      ...
      <Hero
        title={t('pages.lakossagnak.hero_title')}
      ...
      <section className="max-w-7xl mx-auto px-4 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {getTranslationArray<{title: string, desc: string}>(t('pages.lakossagnak.services', { returnObjects: true })).map((s, i) => (
            <Link key={i} to={`/szolgaltatasok/${i === 0 ? 'villamos-felulvizsgalat' : i === 1 ? 'villanyszereles' : 'it-halozat'}`} className="cursor-pointer hover:scale-105 transition-transform">
      ...
      <section className="max-w-7xl mx-auto px-4 pb-20">
        ...
        <ProcessSteps steps={getTranslationArray<{title: string, description: string}>(t('pages.lakossagnak.process_steps', { returnObjects: true })).map((step, i) => ({ number: i + 1, ...step }))} />
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 pb-20">
        ...
        <div className="max-w-lg mx-auto">
          <PricingTable
            rows={pricingRows}
            note={t('common.invoicing.vat_free_individuals')}
          />
        </div>
      </section>
